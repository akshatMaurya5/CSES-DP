...
void solve()
{
    ...
    // dp[i][j]=no. of pages I can get if i select from from first i books and spend j amt;
    ...
    ff(i, 1, n)
    {
        ff(amt, 0, x)
        {
            ...
            int left = amt-price[i-1];
            if(left>=0)
            {
                take=pages[i-1]+dp[i-1][left];
            }
            dp[i][amt]=max(take, nottake);
        }
    }
